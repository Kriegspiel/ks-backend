...
from app.services.bot_usage_game_stats import run_bot_usage_game_stats_migration_once
...
async def _run_bot_usage_game_stats_migration(app: FastAPI) -> None:
    try:
        summary = await run_bot_usage_game_stats_migration_once(app.state.db)
        logger.info("bot_usage_game_stats_migration_complete", **summary)
    except asyncio.CancelledError:
        raise
    except Exception as exc:  # noqa: BLE001
        logger.warning("bot_usage_game_stats_migration_failed", error_type=type(exc).__name__)
...
async def lifespan(app: FastAPI):
    ...
    app.state.bot_usage_game_stats_migration_task = None
    ...
    try:
        ...
        app.state.bot_usage_game_stats_migration_task = asyncio.create_task(
            _run_bot_usage_game_stats_migration(app),
            name="bot-usage-game-stats-migration",
        )
    ...
    try:
        ...
        if migration_task is not None and not migration_task.done():
            ...
            try:
                ...
                pass
        usage_migration_task = getattr(app.state, "bot_usage_game_stats_migration_task", None)
        if usage_migration_task is not None and not usage_migration_task.done():
            usage_migration_task.cancel()
            try:
                await usage_migration_task
            except asyncio.CancelledError:
                pass
        await close_db()
