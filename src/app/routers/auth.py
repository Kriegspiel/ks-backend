...
async def register(
    ...
    try:
        user = await user_service.create_user(payload, acquisition=attribution)
...
async def register_bot(payload: BotRegisterRequest) -> BotRegisterResponse:
    ...
    user_service = UserService(db.users)
    try:
        user, token = await user_service.create_bot(payload)
    except UserConflictError as exc:
        raise _conflict_response(exc) from exc
...
async def login_as_guest(
    ...
    attribution = await attribution_snapshot_from_request(request, analytics_service)
    try:
        user = await user_service.create_guest_user(acquisition=attribution)
    except UserConflictError as exc:
        raise HTTPException(
