...
async def create_game(
    ...
    try:
        return await game_service.create_game(
            ...
            llm_bot_tier=getattr(user, "llm_bot_tier", None),
...
async def join_game(
    game_code: str, user: UserModel = Depends(get_current_user), game_service: GameService = Depends(get_game_service)
) -> Any:
    try:
...
async def get_game_state(
    game_id: str, user: UserModel = Depends(get_current_user), game_service: GameService = Depends(get_game_service)
) -> Any:
    try:
...
async def get_my_archived_games(
    user: UserModel = Depends(get_current_user), game_service: GameService = Depends(get_game_service)
) -> Any:
    try:
        return MyGamesResponse(games=await game_service.get_my_archived_games(user_id=user.id))
    except GameServiceError as exc:
        return _map_game_error(exc)
...
async def get_my_games(
    ...
    try:
        return MyGamesResponse(games=await game_service.get_my_games(user_id=user.id))
    except GameServiceError as exc:
        return _map_game_error(exc)
...
async def get_recent_games(
    limit: int = 10, _: UserModel = Depends(get_current_user), game_service: GameService = Depends(get_game_service)
) -> Any:
    try:
...
async def get_game(
    game_id: str, _: UserModel = Depends(get_current_user), game_service: GameService = Depends(get_game_service)
) -> Any:
    try:
